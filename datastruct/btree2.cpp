...
// 判断一个序列是否符合二叉排序的搜索序列 记录下转折点
int JudgeArrBST(int a[],int length){
    printf("%d\n",length);
    if(length<=2){
        return 1;
    }
    int min=a[0],max=a[0],cmin=-999,cmax=999,i = length,k=1;
    
    int leftOrRight = a[0]>a[1]?left:right;//根据开头两个节点决定树的初步走向
    for(i=2;i<length;i++){
        // 记录下拐点
        int see = a[i];
        if(a[i]<=a[k]){
            // 由右向左拐时，记录转折点的信息，可用递归解决
    ...
    void testJudgeArrBST(){
        ...
        int arr2[] = {20,30,90,50,40,31,32,34,39,35};
        printf("%d\n",JudgeArrBST(arr,sizeof(arr)/sizeof(arr[0])));
        printf("%d\n",JudgeArrBST(arr2,sizeof(arr2)/sizeof(arr2[0])));
    }
    int main()
    {
        testJudgeArrBST();
